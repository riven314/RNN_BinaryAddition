...
def wrap_train(filename, batch_size, n_mb, n_epoch, is_flip, num4test,
    ...
    for i in range(n_epoch):
        ...
        for j in range(n_mb):
            ...
            if is_print_metrics:
                print('train cost: %.4f, test cost: %.4f | train acc: %.4f, test acc: %.4f' % 
                      (train_bs_cost, test_all_cost, train_acc, test_acc))
    sess.close()
    
    # print out metrics and plot
    get_metrics_plot(train_cost_list, test_cost_list, train_acc_list, test_acc_list)
    print('test accuracy: %.4f' % test_acc_list[-1])
    print('test loss: %.4f' % test_cost_list[-1])
    return None
